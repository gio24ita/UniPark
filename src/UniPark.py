...
# ==================== MODELLO DATI (MODEL) ====================
# Gestione logica dei dati (Thread-Safe)
...
class ParkingZone:
    ...
    # Operazione per occupare i posti liberi (parcheggio)
    def park(self):
        with self.lock:
    ...
    # Operazione per liberare posti occupati (uscita dal parcheggio)
    def unpark(self):
        with self.lock:
            if self.waiting > 0:
                self.waiting -= 1
...
# ==================== GUI APPLICATION (VIEW & CONTROLLER) ====================
